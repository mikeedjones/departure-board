...
- [Raspberry Pi: Connecting to the Internet with Raspberry Pi Pico W (PDF)](https://pip.raspberrypi.com/documents/RP-008257-DS)
  Official guide to Wi-Fi (and Bluetooth) on the Pico W family, MicroPython and C. Ch.3 (§3.1-3.4) covers W-series-specific basics ahead of any networking: flashing the UF2, connecting over USB (Thonny/minicom/mpremote), and the on-board LED being wired to the wireless chip's `WL_GPIO0` rather than the RP2350. §3.6 "Connecting to a wireless network" is the primary source for lesson 3 — `network.WLAN(STA_IF)`, the connect/poll sequence, and the CYW43 link-status codes table. Use for: Wi-Fi bring-up now, HTTP/JSON work later.
